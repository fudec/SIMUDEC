function dibujarchanc(hObject,~)
    global cx cy k ck Info cu v a b axesgnu  cumc elec 
     ...
     axis([0 a 0 b])
     f_chanc=figure;
     set(f_chanc,'name','HPGR','numbertitle','off','Position', [500,200,150,180],'Units','pixels','menubar','none','units','normalized','closereq',@Cancelar_callback,...
         'Resize','off')
...
Cancelar = uicontrol('Parent',bg,...
 ...
 waitfor(f_chanc)
    ...
    Chanc_image=imread('HPGR_plot.png'); %%% Im�genes se leen rotadas
    hold on
    ...
    m1 = uimenu(c,'Label','Add Model','Callback',@HPGR_modelos);
    m2 = uimenu(c,'Label','Edit Model','Callback',@HPGR_modelos_edit);
    m3 = uimenu(c,'Label','Delete Module','Callback',@Eliminar_modulo_HPGR);
    m4 = uimenu(c,'Label','Resuls','Callback',@HPGR_Result_GUI);
