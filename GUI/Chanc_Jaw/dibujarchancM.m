function dibujarchancM(hObject,~)
    global cx cy k ck Info cu v a b axesgnu  cumc elec 
     ...
     axis([0 a 0 b])
     f_chanc=figure;
     set(f_chanc,'name','Jaw Crusher','numbertitle','off','Position', [500,200,150,180],'Units','pixels','menubar','none','units','normalized','closereq',@Cancelar_callback,...
         'Resize','off')
...
Cancelar = uicontrol('Parent',bg,...
 ...
 waitfor(f_chanc)
    ...
    Chanc_image=imread('Jaw_plot.jpg'); %%% Im�genes se leen rotadas
    hold on
    p1=plot(cx(k)-6,cy(k)+alto*1.2,'s', 'Markersize',10,'LineWidth',1,'color', 105*ones(3,1)/256); %% Entrada
    p2=plot(cx(k)-6,cy(k)-alto/1.4,'s', 'Markersize',10,'LineWidth',1,'color',160*ones(3,1)/256); %% Salida
    cxd = [cx(k)-6+5,cy(k)+alto*1.2+5];
    cyd = [cx(k)-6+5,cy(k)-alto/1.4-5];
    ...
    m2 = uimenu(c,'Label','Editar Modelo','Callback',@Crusher_modelos_edit);
