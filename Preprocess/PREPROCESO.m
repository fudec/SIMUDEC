...
for i=1:length(S)
    for j=1:length(Info)
            if isfield(Info(j),'v') 
                    if isfield(Info(j).v,'val') 
                        if isfield(Info(j).v,'k')
                            if ~isempty(Info(j).v)
                               if isfield('weigth',S(i).Streams)
                                if ~isempty(S(i).Streams.weigth)
                                    if S(i).Streams.weigth == Info(j).v.k
                                        S(i).Streams.weigth
                                       S(i).Streams.Value=Info(j).v.val;
    %                                    S(i).Streams.WaterFlow=Info(j).v.FW;
                                    end
                                end
                               end
                            end
