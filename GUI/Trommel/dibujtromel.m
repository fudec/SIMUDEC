function dibujtromel(hObject,~)
 ...
 global cx cy k ck Info cu  axesgnu a b elec  cuh  v state
    ...
    axis([0 a 0 b])
    f_hid=figure;
    set(f_hid,'name','Trommel','numbertitle','off','Position', [500,200,150,280],'Units','pixels','menubar','none','units','normalized','closereq',@Cancelar_callback)
...
Cancelar = uicontrol('Parent',bg,...
 ...
 waitfor(f_hid)
    ...
    if strcmp(elec,'Izquierda')== 1
        ...
        hold on
        SCR_image=imread('Tromel_I_plot.jpg');
        p1=plot(cx(k)+45,cy(k)-60,'sk', 'Markersize',10,'LineWidth',1); %%% Sobre tama�o
        p2=plot(cx(k)+10,cy(k)-alto/1.7,'sk', 'Markersize',10,'LineWidth',1); %%% Bajo tama�o
        
        Sobre = [cx(k)+45,cy(k)-60];
        Bajo=[cx(k)+10,cy(k)-alto/1.7];
        
        Info(cu).ioc=[Bajo+5 0;Sobre+5 0;cxd-5 cyd 1];
    ...
    if strcmp(elec,'Derecha')== 1
        ...
        cyd=cy(k)-50;
        p3=plot(axesgnu,cxd,cyd,'sk', 'Markersize',10,'LineWidth',1);        
        SCR_image=imread('Tromel_D_plot.jpg');
        p1=plot(cx(k)-23,cy(k)-60,'sk', 'Markersize',10,'LineWidth',1); %%% Sobre tama�o
        p2=plot(cx(k)+20,cy(k)-alto/1.7,'sk', 'Markersize',10,'LineWidth',1); %%% Bajo tama�o
        
        Sobre = [cx(k)-23,cy(k)-60];
        Bajo = [cx(k)+20,cy(k)-alto/1.7];
    ...
    SCR_image=imresize(SCR_image, [largo largo*1.2]);
